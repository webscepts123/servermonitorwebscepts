...
<div class="space-y-6">
    ...
    <div class="rounded-3xl bg-gradient-to-r from-slate-950 via-blue-950 to-slate-900 p-6 text-white shadow-xl">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div>
                <h1 class="text-3xl font-black">WordPress Manager</h1>
                <p class="text-slate-300 mt-2">
                    Manage WordPress websites from cPanel accounts, check plugins, themes, core and security status.
                </p>
            </div>
            ...
            @if(Route::has('servers.index'))
                <a href="{{ route('servers.index') }}"
                   class="px-5 py-3 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-center">
                    <i class="fa-solid fa-server mr-2"></i>View Servers
                </a>
            @endif
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-5">
        <div class="bg-white rounded-3xl shadow p-6 border">
            <p class="text-slate-500 font-semibold">Servers</p>
            <h2 class="text-4xl font-black mt-2">{{ $servers->count() }}</h2>
        </div>

        <div class="bg-white rounded-3xl shadow p-6 border">
            <p class="text-slate-500 font-semibold">cPanel</p>
            <h2 class="text-4xl font-black mt-2 text-blue-600">
                {{ $servers->where('panel_type', 'cpanel')->count() }}
            </h2>
        </div>

        <div class="bg-white rounded-3xl shadow p-6 border">
            <p class="text-slate-500 font-semibold">Plesk</p>
            <h2 class="text-4xl font-black mt-2 text-purple-600">
                {{ $servers->where('panel_type', 'plesk')->count() }}
            </h2>
        </div>
    ...
    <div class="bg-white rounded-3xl shadow border overflow-hidden">
        <div class="p-6 border-b">
            <h2 class="text-2xl font-black text-slate-800">WordPress-capable Servers</h2>
            <p class="text-slate-500">Open server, cPanel accounts, or WordPress manager if route exists.</p>
        </div>
